...
public class Launch2P : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetKey (KeyCode.KeypadEnter)) {
			if (count < 40) {
				// スペースキーを入力直後にボールを生成
				if (count == 0 && !BallCollisionDetect2P.detect && Timer.IsGameTime())
					Instantiate (ball, transform.position + ballPos, transform.rotation);
